...
range.insertNode(element);return true;},insertElementIntoSelection:function(element){beforeInsert(this);var editor=this.editor,enterMode=editor.activeEnterMode,selection=editor.getSelection(),range=selection.getRanges()[0],elementName=element.getName(),isBlock=CKEDITOR.dtd.$block[elementName];if(this.insertElementIntoRange(element,range)){range.moveToPosition(element,CKEDITOR.POSITION_AFTER_END);if(isBlock){var next=element.getNext(function(node){return isNotEmpty(node)&&!isBogus(node);});if(next&&next.type==CKEDITOR.NODE_ELEMENT&&next.is(CKEDITOR.dtd.$block)){if(next.getDtd()['#'])
...
selection.selectRanges([range]);afterInsert(this);},setData:function(data,isSnapshot){if(!isSnapshot)
data=this.editor.dataProcessor.toHtml(data);this.setHtml(data);if(this.status=='unloaded')
...
ev.data.preventDefault();});}
if(CKEDITOR.env.webkit){this.attachListener(editor,'key',function(evt){var key=evt.data.domEvent.getKey();if(!(key in backspaceOrDelete))
...
if(!this.is('textarea')){var doc=this.getDocument(),head=doc.getHead();if(head.getCustomData('stylesheet')){var refs=doc.getCustomData('stylesheet_ref');if(!(--refs)){doc.removeCustomData('stylesheet_ref');var sheet=head.removeCustomData('stylesheet');sheet.remove();}else
...
function afterInsert(editable){var editor=editable.editor;editor.getSelection().scrollIntoView();setTimeout(function(){editor.fire('saveSnapshot');},0);}
...
bogus.remove();range.deleteContents();if(endBlock.getParent()){endBlock.moveChildren(startBlock,false);startPath.lastElement.mergeSiblings();pruneEmptyDisjointAncestors(startBlock,endBlock,true);}
range=editor.getSelection().getRanges()[0];range.collapse(1);range.select();return true;}
