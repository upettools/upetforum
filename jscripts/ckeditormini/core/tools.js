...
(function(){var functions=[],cssVendorPrefix=CKEDITOR.env.gecko?'-moz-':CKEDITOR.env.webkit?'-webkit-':CKEDITOR.env.ie?'-ms-':'',ampRegex=/&/g,gtRegex=/>/g,ltRegex=/</g,quoteRegex=/"/g,ampEscRegex=/&amp;/g,gtEscRegex=/&gt;/g,ltEscRegex=/&lt;/g,quoteEscRegex=/&quot;/g;CKEDITOR.on('reset',function(){functions=[];});CKEDITOR.tools={arrayCompare:function(arrayA,arrayB){if(!arrayA&&!arrayB)
...
return retval.join('');},htmlEncode:function(text){return String(text).replace(ampRegex,'&amp;').replace(gtRegex,'&gt;').replace(ltRegex,'&lt;');},htmlDecode:function(text){return text.replace(ampEscRegex,'&').replace(gtEscRegex,'>').replace(ltEscRegex,'<');},htmlEncodeAttr:function(text){return text.replace(quoteRegex,'&quot;').replace(ltRegex,'&lt;').replace(gtRegex,'&gt;');},htmlDecodeAttr:function(text){return text.replace(quoteEscRegex,'"').replace(ltEscRegex,'<').replace(gtEscRegex,'>');},getNextNumber:(function(){var last=0;return function(){return++last;};})(),getNextId:function(){return'cke_'+this.getNextNumber();},override:function(originalFunction,functionBuilder){var newFn=functionBuilder(originalFunction);newFn.prototype=originalFunction.prototype;return newFn;},setTimeout:function(func,milliseconds,scope,args,ownerWindow){if(!ownerWindow)
...
return true;}
return false;},transparentImageData:'data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw=='};})();