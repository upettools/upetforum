...
lastChild.remove();}}
if(!this._.nextNode)
this._.nextNode=(isLast||block.equals(lastNode)||!lastNode)?null:getNextSourceNode(block,1,lastNode);return block;}};function startIterator(){var range=this.range.clone(),touchPre,startPath=range.startPath(),endPath=range.endPath(),startAtInnerBoundary=!range.collapsed&&rangeAtInnerBlockBoundary(range,startPath.block),endAtInnerBoundary=!range.collapsed&&rangeAtInnerBlockBoundary(range,endPath.block,1);range.shrink(CKEDITOR.SHRINK_ELEMENT,true);if(startAtInnerBoundary)
