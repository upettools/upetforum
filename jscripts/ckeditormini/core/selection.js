...
html.on('mousedown',function(evt){evt=evt.data;function onHover(evt){evt=evt.data.$;if(textRng){var rngEnd=body.$.createTextRange();moveRangeToPoint(rngEnd,evt.clientX,evt.clientY);textRng.setEndPoint(startRng.compareEndPoints('StartToStart',rngEnd)<0?'EndToEnd':'StartToStart',rngEnd);textRng.select();}}
...
editable.attachListener(editable,'keydown',getOnKeyDownListener(editor),null,null,-1);});editor.on('setData',function(){editor.unlockSelection();if(CKEDITOR.env.webkit)
...
el.remove();},null,null,100);editor.on('key',function(evt){if(editor.mode!='wysiwyg')
...
else if(CKEDITOR.env.ie){var active=this.document.getActive();if(!isMSSelection){var anchorNode=sel&&sel.anchorNode;if(anchorNode)
...
return;var nativ=this.getNative();try{nativ&&nativ[isMSSelection?'empty':'removeAllRanges']();}
catch(er){}
this.reset();}};})();